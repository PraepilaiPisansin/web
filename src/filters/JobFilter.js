...
export default class JobFilter extends Component {
    ...
    handleChangeJob = e => {
        this.setState({ jobActive: e.target.value });
    };
    ...
    render() {
        ...
        const uniqueJob = this.getUnique(this.state.people, "jobs");

        const people = this.state.people;
        const jobActive = this.state.jobActive;

        const jobDropdown = people.filter((data) => {
            return data.jobs === jobActive;
        }).map((data, i) => {
            return <PeopleList obj={data} key={i} />
        });
        const jobLength = jobDropdown.length;
        ...
        return (
            ...
            <Sort
                ...
                <select
                    ...
                    value={this.state.jobActive}
                    onChange={this.handleChangeJob}
                >
                    {uniqueJob.sort((a, b) => (b.jobs < a.jobs ? 1 : -1))
                        .map(c => (
                            <option key={c.id} value={c.jobs}>
                                {c.jobs}
                            </option>
                        ))}
                </select>
