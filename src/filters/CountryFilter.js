...
export default class CountryFilter extends Component {
  ...
  selectCountry = e => {
    this.setState({ countryActive: e.target.value });
  };
  ...
  render() {
    ...
    const uniqueCountry = this.getUnique(this.state.people, "country");

    const people = this.state.people;
    const countryActive = this.state.countryActive;

    const countryDropdown = people.filter((data) => {
      return data.country === countryActive;
    }).map((data, i) => {
      return <PeopleList obj={data} key={i} />
    });

    const countryLength = countryDropdown.length;
    return (
      <main class="main">
        <Sort
          count={countryLength}
        ...
        >
          {uniqueCountry.sort((a, b) => (b.country < a.country ? 1 : -1))
            .map(c => (
